<template>
	<div>
						...
						<div class="profile__content">
							...
							<div class="profile__user">
								...
								<div v-if="this.user.pan" class="profile__meta profile__meta--green">
									...
									<span>UZDUB ID: {{ this.user.id }}</span>
								</div>
							...
							<ul class="nav nav-tabs profile__tabs" id="profile__tabs" role="tablist">
								<li class="nav-item" role="presentation">
									<button id="1-tab" class="active" data-bs-toggle="tab" data-bs-target="#tab-1" type="button" role="tab"
										aria-controls="tab-1" aria-selected="true">PROFIL</button>
								</li>

								<li class="nav-item" role="presentation">
									<button id="2-tab" data-bs-toggle="tab" data-bs-target="#tab-2" type="button" role="tab"
										aria-controls="tab-2" aria-selected="false">IZOHLAR</button>
								</li>

								<li class="nav-item" role="presentation">
									<button id="3-tab" data-bs-toggle="tab" data-bs-target="#tab-3" type="button" role="tab"
										aria-controls="tab-3" aria-selected="false">Sharhlar</button>
								</li>
							</ul>
					...
					<div class="tab-content">
									...
									<div class="col-12 col-lg-6">
										<form action="#" class="sign__form sign__form--profile">
											<div class="row">
												...
												<div class="col-12">
													<button @click="UserEdit()" class="sign__btn sign__btn--small"
														type="button"><span>SAQLASH</span></button>
												</div>
											</div>
										</form>
									</div>
									...
									<div class="col-12 col-lg-6">
										<form action="#" class="sign__form sign__form--profile">
											<div class="row">
												...
												<div class="col-12">
													<button @click="userResetPassword()" class="sign__btn sign__btn--small"
						...
						<div class="tab-pane fade" id="tab-2" role="tabpanel" aria-labelledby="2-tab" tabindex="0">
							...
							<div class="col-12">
								<div class="catalog catalog--1">
									<table class="catalog__table">
										...
										<tbody>
											<tr v-for="(item, index) in user.commentLength"
												...
												<td>
													<div class="catalog__text">
														<NuxtLink to="">{{ item.cinemaName }}</NuxtLink>
													</div>
												</td>
												<td>
												...
												<td>
													<div class="catalog__btns">
														<button @click="WiewModal(item.id)" type="button" data-bs-toggle="modal"
														...
														<button @click="Idfunc(item.id)" type="button" data-bs-toggle="modal"
							...
							<div class="col-12">
								<div class="main__paginator">
									<!-- amount -->
									<span class="main__paginator-pages">{{ this.user.commentLength ? (this.user.commentLength).length : "0" }}
										tadan 10 tasi</span>
									<!-- end amount -->
									...
									<ul class="paginator">
										<li class="paginator__item paginator__item--prev">
											<button @click="count_minus()"><svg xmlns="http://www.w3.org/2000/svg"
										...
										<div v-for="item in length_page" :key="item">
											<li v-if="item != select_page" class="paginator__item">
												<button @click="get_page(item)" v-if="item == 1 || item + 1 == select_page || item - 1 == select_page">{{
													item }}</button>
						...
						<div class="tab-pane fade" id="tab-3" role="tabpanel" aria-labelledby="3-tab" tabindex="0">
							...
							<div class="col-12">
								<div class="catalog catalog--2">
									<table class="catalog__table">
										...
										<tbody>
											<tr v-for="(item, index) in user.sharhLength"
												v-if="index < select_page1 * page_card1 && index >= (select_page1 - 1) * page_card1">
												<td>
													<div class="catalog__text">{{ item.id }}</div>
												</td>
												<td>
													<div class="catalog__text">
														<NuxtLink to="">{{ item.cinemaName1 }}</NuxtLink>
													</div>
												</td>
												<td>
												...
												<td>
													<div class="catalog__btns">
														<button @click="WiewModal1(item.id)" type="button" data-bs-toggle="modal"
														...
														<button @click="Idfunc(item.id)" type="button" data-bs-toggle="modal"
															class="catalog__btn catalog__btn--delete" data-bs-target="#modal-delete2">
															<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
																<path
																	d="M20,6H16V5a3,3,0,0,0-3-3H11A3,3,0,0,0,8,5V6H4A1,1,0,0,0,4,8H5V19a3,3,0,0,0,3,3h8a3,3,0,0,0,3-3V8h1a1,1,0,0,0,0-2ZM10,5a1,1,0,0,1,1-1h2a1,1,0,0,1,1,1V6H10Zm7,14a1,1,0,0,1-1,1H8a1,1,0,0,1-1-1V8H17Z" />
															</svg>
							...
							<div class="col-12">
								<div class="main__paginator">
									<!-- amount -->
									<span class="main__paginator-pages">{{ this.user.sharhLength ? (this.user.sharhLength).length : "0" }} tadan
										10 tasi</span>
									<!-- end amount -->
									...
									<ul class="paginator">
										<li class="paginator__item paginator__item--prev">
											<button @click="count_minus1()"><svg xmlns="http://www.w3.org/2000/svg"
													enable-background="new 0 0 24 24" viewBox="0 0 24 24">
													<path
														d="M8.5,12.8l5.7,5.6c0.4,0.4,1,0.4,1.4,0c0,0,0,0,0,0c0.4-0.4,0.4-1,0-1.4l-4.9-5l4.9-5c0.4-0.4,0.4-1,0-1.4c-0.2-0.2-0.4-0.3-0.7-0.3c-0.3,0-0.5,0.1-0.7,0.3l-5.7,5.6C8.1,11.7,8.1,12.3,8.5,12.8C8.5,12.7,8.5,12.7,8.5,12.8z" />
												</svg></button>
										...
										<div v-for="item in length_page1" :key="item">
											<li v-if="item != select_page1" class="paginator__item">
												<button @click="get_page1(item)" v-if="item == 1 || item + 1 == select_page1 || item - 1 == select_page1">{{
													item }}</button>
										...
										<li class="paginator__item paginator__item--next">
											<button @click="count_plus1()"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
													<path
														d="M15.54,11.29,9.88,5.64a1,1,0,0,0-1.42,0,1,1,0,0,0,0,1.41l4.95,5L8.46,17a1,1,0,0,0,0,1.41,1,1,0,0,0,.71.3,1,1,0,0,0,.71-.3l5.66-5.65A1,1,0,0,0,15.54,11.29Z" />
												</svg></button>
										</li>
		...
		<!-- view modal -->
		<div v-for="item in userIzoh" class="modal fade" id="modal-view" tabindex="-1" aria-labelledby="modal-view"
			aria-hidden="true">
					...
					<div class="modal__content">
						<form action="#" class="modal__form">
							...
							<div class="modal__btns">
								<button @click="userCommentDelete()" class="modal__btn modal__btn--apply"
									type="button"><span>O'chirish</span></button>
		...
		<!-- view modal -->
		<div v-for="item in userSharh" class="modal fade" id="modal-view2" tabindex="-1" aria-labelledby="modal-view2"
			aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal__content modal__content--view">
		...
		<div class="modal fade" id="modal-delete2" tabindex="-1" aria-labelledby="modal-delete2" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal__content">
						<form action="#" class="modal__form">
							<h4 class="modal__title">Sharhlarni o'chirish</h4>

							<p class="modal__text">Bu sharhni butunlay oʻchirib tashlamoqchimisiz?</p>

							<div class="modal__btns">
								<button @click="userReviewDelete()" class="modal__btn modal__btn--apply"
									type="button"><span>Oʻchirish</span></button>
								<button class="modal__btn modal__btn--dismiss" type="button" data-bs-dismiss="modal"
									aria-label="Close"><span>Qoldirish</span></button>
							</div>
						</form>
				...
				<div class="modal-content">
					<div class="modal__content">
						<form action="#" class="modal__form">
							...
							<div class="modal__btns">
								<button @click="UserBanned()" class="modal__btn modal__btn--apply"
									type="button"><span>Apply</span></button>
								<button class="modal__btn modal__btn--dismiss" type="button" data-bs-dismiss="modal"
				...
				<div class="modal-content">
					<div class="modal__content">
						<form action="#" class="modal__form">
							...
							<div class="modal__btns">
								<button @click="UserDelete()" class="modal__btn modal__btn--apply"
...
export default {
			...
			this.$axios.get('https://api.uzdub.uz/users').then(res => {
				...
				this.$axios.get('https://api.uzdub.uz/api/v1/comment').then(res1 => {
					this.$axios.get('https://api.uzdub.uz/api/v1/sharx').then(res2 => {
						...
						Filter.map(item => {
							this.$axios.get('https://api.uzdub.uz/api/v1/cinema').then(res3 => {
								this.$axios.get("https://api.uzdub.uz/api/v1/comment_mark").then(res4 => {
	...
	methods: {
		...
		UserDelete() {
			this.$axios.delete(`https://api.uzdub.uz/users/${sessionStorage.getItem("userEdit")}`).then(res => {
				document.querySelector("#alert_modal").style="display:block"
				document.querySelector("#alert_text").innerHTML="Ma'lumot o'chirildi"
				setTimeout(()=>{
		...
		UserBanned() {
			...
			this.$axios.put(`https://api.uzdub.uz/panu/${sessionStorage.getItem("userEdit")}`, formdata).then(res => {
				document.querySelector("#alert_modal").style="display:block"
				...
				document.querySelector("#alert_modal").style="display:block"
				document.querySelector("#alert_text").innerHTML="User holati o'zgartirilmadi"
			})
		},
		userResetPassword() {
			var formdata = new FormData()
			...
			this.$axios.put(`https://api.uzdub.uz/reset/${sessionStorage.getItem("userEdit")}`, formdata).then(res => {
				document.querySelector("#alert_modal").style="display:block"
				document.querySelector("#alert_text").innerHTML="User paroli o'zgardi"
				setTimeout(()=>{
					window.location.reload()
				},1000)
			}).catch(err => {
				document.querySelector("#alert_modal").style="display:block"
				document.querySelector("#alert_text").innerHTML="User paroli o'zgarmadi"
			})
		},
		get_page(id) {
			this.select_page = id
		...
		UserEdit() {
			...
			this.$axios.put(`https://api.uzdub.uz/users/${sessionStorage.getItem("userEdit")}`, formdata).then(res => {
				if (document.querySelector("#subscription").value == 0) {
					this.$axios.get('https://api.uzdub.uz/api/pay').then(res => {
						const Filter = res.data.filter(item => item.user_id == sessionStorage.getItem("userEdit"))
						this.$axios.delete(`https://api.uzdub.uz/api/pay/${Filter[0].id}`).then(res => {
					...
					this.$axios.post(`https://api.uzdub.uz/api/pay`, formdata1).then(res => {
					}).catch(err => { console.log(err) })
				}
				document.querySelector("#alert_modal").style="display:block"
				document.querySelector("#alert_text").innerHTML="User ma'lumoti o'zgartirildi"
				setTimeout(()=>{
					window.location.reload()
				},1000)
			}).catch(err => {
				document.querySelector("#alert_modal").style="display:block"
				document.querySelector("#alert_text").innerHTML="User ma'lumoti o'zgartirilmadi"
			})
		},
		AlertNone(){
