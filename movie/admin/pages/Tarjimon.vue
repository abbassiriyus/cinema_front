						...
						<table class="catalog__table">
							...
							<tbody>
								<tr v-for="(item,index) in izoh" ::key="item.id" v-if="index<select_page*page_card && index>=(select_page-1)*page_card">
									...
									<td>
										<div class="catalog__text">{{ item.full_name }}</div>
									</td>
									<td>
									...
									<td>
										<div class="catalog__btns">
											<button @click="WiewModal(item.id)" type="button" data-bs-toggle="modal" class="catalog__btn catalog__btn--edit"
...
export default {
	...
	methods:{
		CommentSearch(){
			this.$axios.get('https://api.uzdub.uz/api/v1/tarjima').then(res=>{
					const a=res.data.filter(item=>((item.full_name).toLowerCase()).includes((document.querySelector("#commentSearch").value).toLowerCase()))
			        this.izoh=a
                    var for_w=this.izoh.length/this.page_card
            		var daad=[]
            		for (let i = 0; i < for_w; i++) {
		...
		WiewModal(id){
        sessionStorage.setItem("tarjimonId",id)
        window.location="add-item1"
		},
		Idfunc(id){
            this.deleteId=id
		},
		CommentDelete(){
           this.$axios.delete(`https://api.uzdub.uz/api/v1/tarjima/${this.deleteId}`).then(res=>{
