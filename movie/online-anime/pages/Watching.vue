		...
		<section class="section section--details">
			...
			<div class="container">
				<div class="row">
					...
					<div class="col-12 col-xl-6">
						<div class="cinema-iframe" v-if="data_get && data_get.video">
							<iframe width="100%" height="100%" src="https://kinescope.io/embed/sppLFtmKoUar14Q2y737wg"
								allow="autoplay; fullscreen; picture-in-picture; encrypted-media;" frameborder="0" allowfullscreen
								style="position: absolute; top: 0; left: 0"></iframe>
						</div>
						<div v-else-if="data_get" class="cinema-iframe">
							<iframe width="100%" height="100%" :src="data_get.treler"
								allow="autoplay; fullscreen; picture-in-picture; encrypted-media;" frameborder="0" allowfullscreen
								style="position: absolute; top: 0; left: 0"></iframe>
						</div>
					</div>
										...
										<div class="comments" v-if="data_get">
											<ul v-for="commnet in data_get.comment" class="comments__list">
												<li class="comments__item">
													<div class="comments__autor">
														...
														<span class="comments__name" v-if="commnet.creators && commnet.creators.name">{{
															commnet.creators.name }} <span v-if="commnet.ptichka"
																class="d-flex aligin-items-center justify-content-center moder-user">
																<img src="img/verify.svg" alt="" /> </span></span>
														<span class="comments__name" v-else>No Name</span>
														<span class="comments__time">
															{{ commnet.time_create.slice(5, 7) }}.{{ commnet.time_create.slice(8, 10) }}.{{
																commnet.time_create.slice(0, 4) }}, {{ commnet.time_create.slice(11, 16) }}</span>
													</div>
													<p class="comments__text">
														{{ commnet.description }}
													</p>
													<div class="comments__actions">
														<div class="comments__rate">
															...
															<button type="button">
																{{ commnet.min }}<svg @click="click_like(false, commnet.id)"
												...
												<div v-for="cinema in commnet.subcomment">
													<li v-if="!cinema.quanty" class="comments__item comments__item--answer">
														<div class="comments__autor">
															...
															<span class="comments__name" v-if="cinema.creators && cinema.creators.name">{{
																cinema.creators.name }} <span v-if="cinema.ptichka"
																	class="d-flex aligin-items-center justify-content-center moder-user">
																	<img src="img/verify.svg" alt="" /> </span></span>
															<span class="comments__name" v-else>No Name</span>
															<span class="comments__time">
																{{ cinema.time_create.slice(5, 7) }}.{{ cinema.time_create.slice(8, 10) }}.{{
																	cinema.time_create.slice(0, 4) }}, {{ cinema.time_create.slice(11, 16) }}</span>
														</div>
														<p class="comments__text">
															{{ cinema.description }}
														</p>
													...
													<li v-else class="comments__item comments__item--quote">
														<div class="comments__autor">
															<img class="comments__avatar" src="img/user.svg" alt="" />
															<span class="comments__name" v-if="cinema.creators && cinema.creators.name">{{
																cinema.creators.name }}
														...
														<div class="comments__actions">
															<div class="comments__rate">
																...
																<button type="button">
																	{{ cinema.min }}<svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 24 24"
																		@click="click_like(false, cinema.id)">
																		<path
																			d="M19,2H6.27A3,3,0,0,0,3.32,4.46l-1.27,7A3,3,0,0,0,5,15H9.56L9,16.43A4.13,4.13,0,0,0,12.89,22a1,1,0,0,0,.91-.59L16.65,15H19a3,3,0,0,0,3-3V5A3,3,0,0,0,19,2ZM15,13.79l-2.72,6.12a2.13,2.13,0,0,1-1.38-2.78l.53-1.43A2,2,0,0,0,9.56,13H5a1,1,0,0,1-.77-.36A1,1,0,0,1,4,11.82l1.27-7a1,1,0,0,1,1-.82H15ZM20,12a1,1,0,0,1-1,1H17V4h2a1,1,0,0,1,1,1Z">
																		</path>
											...
											<ul class="reviews__list">
												<li v-for="sharx in data_get.sharx" class="reviews__item">
													<div class="reviews__autor">
														...
														<span class="reviews__name">{{ sharx.title }}</span>
														<span class="reviews__time"> {{ sharx.time_create.slice(5, 7) }}.{{
															sharx.time_create.slice(8, 10) }}.{{ sharx.time_create.slice(0, 4) }},
															<span v-if="sharx.creators && sharx.creators.name">{{ sharx.creators.name }}</span>
															tomonidan</span>
							...
							<div class="tab-pane fade" id="tab-3" role="tabpanel" aria-labelledby="3-tab" tabindex="0">
								...
								<div class="gallery" itemscope="">
									<div v-if="data_get" class="row row--grid gallery_in" id="lightgallery">
										<!-- gallery item -->
										<a v-for="img in data_get.allimage" :href="img.image" itemprop="contentUrl" data-size="1920x1280">
											<img :src="`${img.image}`" itemprop="thumbnail" alt="project-4" />
										</a>
										<!-- end gallery item -->
									</div>
								</div>
