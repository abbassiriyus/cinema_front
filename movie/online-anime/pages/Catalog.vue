<template>
	<div>
		...
		<div v-else>
					...
					<div class="row">
						<div class="col-12">
							<div class="filter__content">
								<!-- menu btn -->
								<button class="filter__menu" type="button" @click="mfilter__active = 'mfilter--active'">
								...
								<!-- amount -->
								<span class="filter__amount" v-if="catalog">jami {{ catalog.length }} ta anime</span>
								<!-- end amount -->
							</div>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-12">
							<div class="section__paginator">
								<!-- amount -->
								<span class="section__paginator-pages" v-if="catalog">jami {{ catalog.length }} ta anime</span>
								<!-- end amount -->

								<ul class="section__paginator-list">
									<li>
										<a href="#next__btn" @click="count_minus()">
									...
									<li>
										<a href="#next__btn" @click="count_plus()">
											<svg width="24px" height="24px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
												<path
								...
								<ul class="paginator">
									...
									<div v-for=" item  in  length_page " :key="item">
										<li v-if="item != select_page" class="paginator__item">
											<button @click="get_page(item)" v-if="item == 1 ||
												item + 1 == select_page ||
												item - 1 == select_page
												">
												{{ item }}
											</button>
											<button @click="get_page(item)" v-else-if="item == length_page.length">
												{{ item }}
											</button>
			...
			<!-- mobile filter -->
			<div id="mfilter_media" class="mfilter" :class="mfilter__active">
				<div class="mfilter__head">
					<h6 class="mfilter__title">Filter</h6>
					<button class="mfilter__close" type="button" @click="mfilter__active = ''">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
							<path
				...
				<button @click="Allfilter(1)" class="mfilter__apply" type="button"><span>Filter</span></button>
			</div>
			<!-- end mobile filter -->
		</div>
	</div>
